export default async function(sock, plan, context) {
    ...
    if (!isAdmin && !isOwner) throw new Error('Only admins can demote');
    if (!plan.target) throw new Error('No target provided');
    
    await sock.groupParticipantsUpdate(from, [plan.target], 'demote');
    console.log('\x1b[32mGROUP_ACTION:\x1b[0m demote executed on ' + plan.target);
}
